...
RSpec.feature "User::Indicators::ExamsInProgresses", type: :feature do
  ...
  it "with one exam" do
    exam = Exam.create(offered_exam: OfferedExam.all.sample, exam_status_kind: ExamStatusKind.IN_PROGRESS)
    navigate_to
  ...
  it "with canceled" do
    exam = Exam.create(offered_exam: OfferedExam.all.sample, exam_status_kind: ExamStatusKind.IN_PROGRESS)
    exam = Exam.create(offered_exam: OfferedExam.all.sample, exam_status_kind: ExamStatusKind.CANCELED)
    navigate_to
    expect(find(id: "total").text).to eq 1.to_s
  end

  it "with complete" do
    Exam.create(offered_exam: OfferedExam.all.sample, exam_status_kind: ExamStatusKind.IN_PROGRESS)
